...
class TestSNRate(unittest.TestCase):
    ...
    def testGenerateSample(self):
        ...
        params_ref = np.array([(0.1, -2., 0.2, 59022., 0., 0., 0., -15., 30.),
                               (0.2, -2., 0.2, 59024., 0., 0., 0., -15., 30.),
                               (0.3, -2., 0.2, 59026., 0., 0., 0., -15., 30.)], dtype=list(zip(names, types)))
    ...
    def testGenerateFakeObservations(self):
        ...
        dtype = [('observationStartMJD', '<f8'), ('fieldRA', '<f8'), ('fieldDec', '<f8'), ('filter', '<U1'),
                 ...
                 ('season', '<f8'), ('seeingFwhmEff', '<f8'), ('seeingFwhmGeom', '<f8')]
        ref_obs = np.array([(-40., 0., 0., 'g', 23.27, 1, 30., 1., 0.87, 0.87),
                            (-39.9930556, 0., 0., 'r',
                             24.58, 1, 30., 1., 0.83, 0.83),
                            (-39.9861112, 0., 0., 'i', 24.22, 1, 30., 1., 0.8, 0.8),
                            (-39.9791668, 0., 0., 'z',
                             23.65, 1, 30., 1., 0.78, 0.78),
                            (-39.9722224, 0., 0., 'y',
                             22.78, 1, 30., 1., 0.76, 0.76),
                            (-37., 0., 0., 'g', 23.27, 1, 30., 1., 0.87, 0.87),
                            (-36.9930556, 0., 0., 'r',
                             24.58, 1, 30., 1., 0.83, 0.83),
                            (-36.9861112, 0., 0., 'i', 24.22, 1, 30., 1., 0.8, 0.8),
                            (-36.9791668, 0., 0., 'z',
                             23.65, 1, 30., 1., 0.78, 0.78),
                            (-36.9722224, 0., 0., 'y',
                             22.78, 1, 30., 1., 0.76, 0.76),
                            (-34., 0., 0., 'g', 23.27, 1, 30., 1., 0.87, 0.87),
                            (-33.9930556, 0., 0., 'r',
                             24.58, 1, 30., 1., 0.83, 0.83),
                            (-33.9861112, 0., 0., 'i', 24.22, 1, 30., 1., 0.8, 0.8),
                            (-33.9791668, 0., 0., 'z',
                             23.65, 1, 30., 1., 0.78, 0.78),
                            (-33.9722224, 0., 0., 'y',
                             22.78, 1, 30., 1., 0.76, 0.76),
                            (-31., 0., 0., 'g', 23.27, 1, 30., 1., 0.87, 0.87),
                            (-30.9930556, 0., 0., 'r',
                             24.58, 1, 30., 1., 0.83, 0.83),
                            (-30.9861112, 0., 0., 'i', 24.22, 1, 30., 1., 0.8, 0.8),
                            (-30.9791668, 0., 0., 'z',
                             23.65, 1, 30., 1., 0.78, 0.78),
                            (-30.9722224, 0., 0., 'y',
                             22.78, 1, 30., 1., 0.76, 0.76),
                            (-30., 0., 0., 'g', 23.27, 1, 30., 1., 0.87, 0.87),
                            (-29.9930556, 0., 0., 'r',
                             24.58, 1, 30., 1., 0.83, 0.83),
                            (-29.9861112, 0., 0., 'i', 24.22, 1, 30., 1., 0.8, 0.8),
                            (-29.9791668, 0., 0., 'z',
                             23.65, 1, 30., 1., 0.78, 0.78),
                            (-29.9722224, 0., 0., 'y', 22.78, 1, 30., 1., 0.76, 0.76)], dtype=dtype)
    ...
    def testTemplateData(self):
        ...
        """
        refsimu['flux'] = [2.053669674746585e-08, 2.181581633696828e-06, 4.541527969545595e-06, 3.2604225596725335e-06, 1.5571616529418014e-06,
                           6.851255774726093e-07, 3.442408925772686e-07, 2.996352231075117e-07, 2.7455873636949045e-07, 2.460277348387451e-07]
        refsimu['fluxerr'] = [2.3839282850058026e-08, 2.5293560171617316e-08, 2.6791666386123333e-08, 2.5989087700673264e-08,
                              2.4882137361078126e-08, 2.429583660373495e-08, 2.406276362307958e-08, 2.4032099174452836e-08, 2.401484090660781e-08, 2.399520466299214e-08]
        ...
        """
        refsimu['snr_m5'] = [0.8614645363552059, 86.25047715287023, 169.51270981404375, 125.45352100174222, 62.58150698008729,
                             28.19929968442767, 14.305958283490476, 12.468125274134893, 11.432877587539801, 10.253204267025664]

        refsimu['mag'] = [28.1187391467819, 23.053136950507632, 22.257060639313785, 22.6168808986143, 23.419231371953128,
                          24.310640169445442, 25.057909474402514, 25.208583460343878, 25.303477453846973, 25.422605451998646]

        refsimu['magerr'] = [1.2603376679344231, 0.012588176211869204, 0.006405043055173781, 0.008654489695375321,
                             0.017349154041681962, 0.038502240016893, 0.07589398649450159, 0.08708095089568016, 0.09496613572959325, 0.10589238022399108]
