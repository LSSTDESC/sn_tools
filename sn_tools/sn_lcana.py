...
def sigma_x0_x1_color(resu, restab, params=['x0', 'x1', 'color']):
    """
    Method to estimate SN parameters error from Fisher matrix
    ...
    restab: array
       SN parameters
    params: list(str), opt
       list of SN parameters to consider (default =['x0', 'x1', 'color'])
...
def calc_info(lc, params=['x0', 'x1', 'color'], j=-1, output_q=None):
    """
    Function to estimate sn parameters from Fisher elements
    ...
    params: list(str), opt
      list of SN parameters to consider(default=['x0', 'x1', 'color'])
    j: int, opt
       multiprocess index(default: -1)
    output_q: multiprocessing queue, opt
       default: None

    Returns
    ----------
    array with SN parameters
...
class LCtoSN:
    """
    class to transform light curves(from full simulation) to supernovae

    Parameters
    --------------
    inputDir: str
     input directory where LC curves are located
    outputDir: str
     outpur directory for results
    procId: str
      id to identify LC in the hdf5 file
    tempDir: str, opt
      temporary directory(default: 'temp')
    nproc: int, opt
      number of core to use for the processing(defaul=1)
    ...
    def process(self):
        """
        Method to process LC
        The results of this method is a numpy array file of supernovae
    ...
    def lcLoop(self, group, ida, idb, lcName, dirFile, tempDir, j=0, output_q=None):
        ...
        group: pandas df
          group of LC
        ida: int
          index used for multiprocessing
        idb: int
          index used for multiprocessing
        lcName: str
           lc name
        dirFile: str
           directory where lc files are located
        j: int, opt
          multiprocess number(default=0)
        output_q: muleiprocessing queue
          to transmit the results when multiprocessing is activated(default: None)

        Returns
        -----------
        astropy table with SN parameters
    ...
    def calcSN(self, grp, lcFile):
        """
        Method to estimate supernovae from light curves

        Parameters
        ---------------
        grp: astropy Table group
        lcFile: astropy table of LC points
        ...
        for index in grp['id_hdf5']:
            all_lc = vstack([all_lc, getLC(lcFile, index)],
...
def sigma_x0_x1_color_loop(lcList, params=['x0', 'x1', 'color']):
    ...
    for ilc, lct in enumerate(lcList):
        ...
        print(lc[['band', 'phase', 'flux', 'fluxerr',
                  'F_x0x0', 'F_x1x1', 'F_colorcolor', 'snr_m5']])
        parts = np.zeros((3, 3))
        for ia, vala in enumerate(params):
            for jb, valb in enumerate(params):
                if jb >= ia:
